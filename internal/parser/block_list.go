...
func parseListBlocks(ctx *blockContext) *parsedList {
	...
	for !ctx.isEOF() && strings.TrimSpace(ctx.getLine()) != "" {
		...
		if level < currentLevel {
			...
			item.Range = ast.Range{StartLine: ctx.getPos()}
			item.Blocks = append(item.Blocks, parseListBlocks(ctx))
			item.Range.EndLine = ctx.getPos()
		} else {
			item.Blocks = append(item.Blocks, &parsedBlock{
				Type:  "Paragraph",
				Attr:  "",
				Text:  text,
				Range: ast.Range{StartLine: ctx.getPos(), EndLine: ctx.getPos()},
			})
			item.Range = ast.Range{StartLine: ctx.getPos(), EndLine: ctx.getPos()}
		}
	...
	return &parsedList{
		Ordered: ordered,
		Items:   items,
		Range:   ast.Range{StartLine: startLine, EndLine: ctx.getPos()},
	}
}
