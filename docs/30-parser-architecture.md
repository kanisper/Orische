...
Styled Sugar is not committed until a qualifying close is found. An unterminated Styled opener remains Text without advancing the scanner, which allows later valid inline syntax on the same logical line to be recognized. Closed but invalid Styled candidates consume through their close as literal Text. Unterminated Code Span candidates still advance to the logical line end. A Link becomes committed after its unescaped `](` sequence; URI scanning stops at the first unescaped `)` and removes ASCII-punctuation escapes without introducing a general delimiter or backtracking engine.
